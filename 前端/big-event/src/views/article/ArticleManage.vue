...
const imgPath = ref(''); // 保存相对路径
const uploadSuccess = (result) => {
  // const url = result.data
  // const urlObj = new URL(url);
  // urlObj.port = '5173'; // 修改端口
  // urlObj.pathname = urlObj.pathname.replace('/file/', '/api/file/'); // 修改路径
  // articleModel.value.coverImg = urlObj.toString()
  articleModel.value.coverImg = apiTarget+result.data
  imgPath.value = result.data
...
const addArticle = async (clickState) => {
  ...
  articleModel.value.coverImg = imgPath.value
...
const showDrawer = (row) => {
  ...
  articleModel.value.coverImg = apiTarget+row.coverImg
...
//因为直接复制为空字符串 articleModel.value.content = ''，quill-editor 组件不更新,可以使用articleModel.value.content = ' '将其变为空格
//因为quill-editor 组件是富文本编辑器，不能使用空字符串清空，得使用'<p></p>'清空
// const editorKey = ref(0); // 或者在quill-editor 组件中定义一个:key="editorKey"用于强制更新 quill-editor 组件
